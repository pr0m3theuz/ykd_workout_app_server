...
if ! [ -f "$FILE" ]; then
  ...
  collections=("CompletedPrograms" "CompletedSets" "CompletedWorkouts" "Equipment" "Exercise" "ExerciseNote" "Force" "Mechanic" "Mesocycle" "MesocycleNote" "Microcycle" "MicrocycleNote" "ModernizedLoadRIR" "Muscle" "MuscleExercise" "MuscleGroup" "MuscleRoleFunction" "Program" "ProgramNote" "LoadRepCapacity" "Users" "Utility" "Workout" "WorkoutNote")
  ...
  for name in ${collections[@]}; do
    ...
    --password $COUCHBASE_ADMINISTRATOR_PASSWORD \
    --bucket $COUCHBASE_BUCKET \
    --create-collection _default.$name \
    --max-ttl 0
  done
